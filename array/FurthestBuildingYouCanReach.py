...
def furthestBuilding(heights: list([int]), bricks: int, ladders: int) -> int:
    largestHeights = [0 for _ in range(ladders)]
    result = 0

    for i in range(1,len(heights)):
        curHeight = heights[i] - heights[i-1]
        if curHeight > 0:
            if largestHeights and largestHeights[0] < curHeight and largestHeights[0] <= bricks:
                bricks -= heappushpop(largestHeights, curHeight)
            elif curHeight <= bricks:
                # result += 1
                bricks -= curHeight
            else:
                break
